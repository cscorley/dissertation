...
then
    ...
    RESPONSE=`curl --silent "https://www.googleapis.com/oauth2/v4/token" --data "client_id=$CLIENT_ID&client_secret=$CLIENT_SECRET&code=$DEVICE_CODE&grant_type=http://oauth.net/grant_type/device/1.0"`
    echo $RESPONSE
...
if [ -z "$ACCESS_TOKEN" ]
    ...
    RESPONSE=`curl --silent "https://www.googleapis.com/oauth2/v4/token" --data "client_id=$CLIENT_ID&client_secret=$CLIENT_SECRET&refresh_token=$REFRESH_TOKEN&grant_type=refresh_token"`
    echo $RESPONSE
    ACCESS_TOKEN=`echo $RESPONSE | jsonValue access_token`
fi
